/**
 * Flocking background — full-page ASCII boids ecosystem
 * Adapted from personal-website/flock-bg.js
 */
(function () {
  ...
  const canvas = document.getElementById('flock-bg');
  if (!canvas) return;

  // Allow pointer events on the canvas for mouse tracking
  ...
  document.addEventListener('mousemove', function (e) {
    ...
    if (moved > 2) {
      ...
      if (moved > 8 && cellW > 0) {
        ...
        const exists = crumbs.some(function (c) { return c.gx === gx && c.gy === gy; });
        if (!exists) {
          crumbs.push({ x: snapX, y: snapY, gx: gx, gy: gy, strength: 1.0 });
  ...
  function step() {
    ...
    boids.push.apply(boids, newBoids);
  }
